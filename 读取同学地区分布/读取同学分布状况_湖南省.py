...
read_data('sheet1')  # 调用方法，并传入表名
#
import csv
with open("sjxx1.csv","w") as f:
    writer = csv.writer(f)
    for row in list:
        writer.writerow(row)

df = pd.read_csv("sjxx1.csv", encoding='gbk')
...
hunan_city = Counter(df.所在市)
print(hunan_city)
area = [(i[0],i[1]) for i in hunan_city.items()]
maps = (
        ...
        .add("居住地", area, "湖南")
        .set_global_opts(
            title_opts=opts.TitleOpts(title="电商一班湖南省同学所在市级分布"),
